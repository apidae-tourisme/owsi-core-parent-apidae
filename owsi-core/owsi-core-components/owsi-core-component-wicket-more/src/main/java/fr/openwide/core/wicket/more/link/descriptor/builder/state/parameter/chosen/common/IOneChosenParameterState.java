 ...
 * <p>On top of the methods provided by {@link IChosenParameterState}, this state provides several mapping and
 * validation methods that only make sense when applied to a single model.
 */
public interface IOneChosenParameterState
	...
	/**
	 * Map the HTTP query parameter with the given name to the currently selected model, making sure that:
	 * <ul>
	 * <li>when {@link ILinkParametersExtractor extracting parameters} from a HTTP query, the parameter with the given
	 * name will get converted and stored in the currently selected model
	 * <li>when {@link ILinkGenerator generating a link's URL}, the value stored in the currently selected model will
	 * get converted and assigned to the HTTP query parameter with the given name.
	 * </ul>
	 * <p><strong>Note:</strong> collection mapping requires calling {@link #mapCollection(String, Class)} instead.
	 ...
	 * <strong>is not written to the currently selected model</strong>.
	 * <li>when {@link ILinkGenerator generating a link's URL}, the value stored in the currently selected model will
	 * get converted and assigned to the HTTP query parameter with the given name.
	 * </ul>
	 * <p>This is useful when dealing with interrelated HTTP query parameters where one parameters will fully determine
	 * another, in which case the latter will only be here for cosmetics.
	 * @param parameterName The HTTP query parameter name.
	 ...
	 * pass validation if and only if the given predicate is <code>true</code> when applied to the currently selected
	 * model's object, and raise an error otherwise.
	 ...
	 * currently selected model, and raise an error otherwise.
	 ...
	 * contained in the currently selected model, and raise an error otherwise.
	...
	/**
	 * Similar to {@link #permission(String, String...)}, but checks permissions on the object pointed to by the given
	 * binding on the currently selected model's object, instead of simply checking them on the currently selected
	 * model's object.
	 */
